...
class PolaAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, sin_l: torch.Tensor, cos_l: torch.Tensor):
        ...
        q,k,v,g  = self.qkvg(x).reshape(B, N, 4, C).permute(2,0,1,3).unbind(0)
        s1 = torch.sigmoid(self.s1)
        s2 = torch.sigmoid(self.s2)

        lepe = self.lepe(v.reshape(B, H, W, C).permute(0,3,1,2).contiguous())
        ...
        res = res.transpose(1,2).reshape(B,H,W,C).permute(0,3,1,2).contiguous()

        res = res + lepe
        res = res.reshape(B,C,N).permute(0,2,1)

        res = self.proj(res * g).reshape(B,H,W,C).permute(0,3,1,2).contiguous()
...
class ASSM(nn.Module):
    def __init__(self, dim, d_state, input_resolution, num_tokens=64, inner_rank=128, mlp_ratio=2.):
        ...
        # self.act = nn.SiLU()
        self.out_proj = nn.Linear(self.dim, dim, bias=True)
        ...
        self.CPE = nn.Sequential(
            nn.Conv2d(self.dim, self.dim, 3, 1, 1, groups=self.dim),
        )
        ...
        num_head = 4
        self.attn = PolaAttention(dim=self.dim, num_heads=num_head)
        self.RoPE_linear = RoPE(2 * self.dim, num_head)
    ...
    def forward(self, x, x_size):
        ...
        y = self.attn(x, sin_l, cos_l)

        y = y.view(B, C, -1).permute(0, 2, 1).contiguous()  # b,n,c
...
class AttentiveLayer(nn.Module):
    ...
    def forward(self, x, x_size, params):
        ...
        shortcut = x
        # print("1", x.shape)
        # print("2", self.assm(self.norm3(x), x_size, self.embeddingA).shape)
        # exit()
