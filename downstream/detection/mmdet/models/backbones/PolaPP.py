...
class RoPE(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, base=10000):
        ...
        '''
        super().__init__()
        angle = 1.0 / (base ** torch.linspace(0, 1, embed_dim // num_heads // 4))
...
class AddLinearAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads):
        ...
        self.relu = nn.ReLU()
        self.s1 = nn.Parameter(torch.zeros(size=(1, self.num_heads, 1, 2 * self.head_dim)))
        self.s2 = nn.Parameter(torch.zeros(size=(1, self.num_heads, 1, 2 * self.head_dim)))
        


    def forward(self, x: torch.Tensor, sin_l: torch.Tensor, cos_l: torch.Tensor):
        '''
        x: (b c h w)
        sin: ((h w) d1)
        cos: ((h w) d1)
        ...
        q,k,v,g  = self.qkvg(x).reshape(B, N, 4, C).permute(2,0,1,3).unbind(0) #(b 3*c h w)
        s1 = torch.sigmoid(self.s1)
        s2 = torch.sigmoid(self.s2)

        lepe = self.lepe(v.reshape(B, H, W, C).permute(0,3,1,2)).contiguous() # (b c h w)
        ...
        q_sim = torch.exp(torch.cat([q_pos, q_neg],dim=-1)*s1) - 1
        q_opp = torch.exp(torch.cat([q_neg, q_pos],dim=-1)*s2) - 1
        ...
        k0 = torch.exp(k) - 1
        k = k0
        k = theta_shift(k, sin_l, cos_l)
        
        q = q_sim
        z = q @ k0.mean(dim=-2, keepdim=True).transpose(-2, -1)
        q = theta_shift(q, sin_l, cos_l)
        kv = (k.transpose(-2, -1) * (1 / (H*W)) ** 0.5) @ (v1 * (1 / (H*W)) ** 0.5)
        res1 = q @ kv * (1/(z + 1e-6))

        q = q_opp
        z = q @ k0.mean(dim=-2, keepdim=True).transpose(-2, -1)
        q = theta_shift(q, sin_l, cos_l)
        kv = (k.transpose(-2, -1) * (1 / (H*W)) ** 0.5) @ (v2 * (1 / (H*W)) ** 0.5)
        res2 = q @ kv * (1/(z + 1e-6))
        
        res = torch.cat([res1, res2],dim=-1)
        res = res.transpose(1,2).reshape(B,H,W,C).permute(0,3,1,2)

        res = res + lepe
        res = res.reshape(B,C,N).permute(0,2,1)

        res = self.proj(res * g).reshape(B,H,W,C).permute(0,3,1,2)
...
class VanillaSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, sin: torch.Tensor, cos: torch.Tensor):
        ...
        q,k,v,o  = self.qkvo(x).reshape(B, N, 4, C).permute(2,0,1,3).unbind(0)
        
        lepe = self.lepe(v.reshape(B,H,W,C).permute(0,3,1,2)) # (b c h w)
        ...
        res = self.proj(res * o).reshape(B,H,W,C).permute(0,3,1,2)
        return res
...
class FeedForwardNetwork(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        B, C, H, W = x.shape
        N = H * W
        x = x.reshape(B, C, N).transpose(-1,-2) #B,N,C
        ...
        if self.dwconv is not None:
            ...
            x = self.dwconv(x)
            x = x + residual
            x = x.reshape(B, self.ffn_dim, N).transpose(-1,-2) #B,N,C
        x = self.fc2(x)
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, sin: torch.Tensor, cos: torch.Tensor, sin_l: torch.Tensor, cos_l: torch.Tensor):
        x = x + self.pos(x)
        if self.flag == 'l':
            if self.layerscale:
                x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x), sin_l, cos_l))
                x = x + self.drop_path(self.gamma_2 * self.ffn(self.norm2(x)))
            else:
                x = x + self.drop_path(self.attn(self.norm1(x), sin_l, cos_l))
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, flags, embed_dim, out_dim, depth, num_heads,
        ...
        self.RoPE_linear = RoPE(2 * embed_dim, num_heads)
    ...
    def forward(self, x: torch.Tensor):
        ...
        sin_l, cos_l = self.RoPE_linear((h, w))
        
        for blk in self.blocks:
            x = blk(x, sin, cos, sin_l, cos_l)
...
class PolaPP(nn.Module):
    ...
    def __init__(self, in_chans=3, out_indices=(0, 1, 2, 3), flagss=[['l']*10, ['l']*10, ['v', 'v']*20, ['v']*10],
                 ...
                 pretrained=None, init_cfg=None, norm_eval=True):
        super().__init__()
        # two_stage / single_stage detectors may inject model.pretrained into backbone cfg
    ...
    def init_weights(self, pretrained=None):
        ...
        elif getattr(self, 'init_cfg', None) is not None:
...
@register_model
def PolaPP_b1(args=None):
    model = PolaPP(
        embed_dims=[32, 64, 192, 384],
        depths=[2, 2, 4, 2],
        num_heads=[1, 2, 6, 12],
